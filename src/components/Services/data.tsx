import tilingImage from "@/../public/tiling.webp"
import flooringImage from "@/../public/flooring-services.webp"
import plasteringImage from "@/../public/plastering-services.webp"
import paintingImage from "@/../public/painting-service.webp"
import bathroomImage from "@/../public/bathroom-renovation.webp"
import kitchenImage from "@/../public/kitchen-renovation.webp"

export const services = [
  {
    name: "Tiling Services",
    url: "/services/tiling",
    description: "High-quality tiling services for homes and businesses.",
    slug: "tiling",
    svg: (
      <>
        {" "}
        <svg
          fill="#ffffff"
          version="1.1"
          id="Capa_1"
          xmlns="http://www.w3.org/2000/svg"
          width="48"
          height="48px"
          viewBox="-4.27 -4.27 51.24 51.24"
          stroke="#ffffff"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0">
            <rect
              x="-4.27"
              y="-4.27"
              width="51.24"
              height="51.24"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <g>
              {" "}
              <path d="M42.629,14.199c-0.006-0.021-0.019-0.04-0.024-0.061c-0.031-0.096-0.07-0.189-0.121-0.278c0-0.004-0.002-0.007-0.004-0.01 c-0.019-0.031-0.041-0.057-0.062-0.088c-0.045-0.07-0.09-0.142-0.143-0.206c-0.04-0.046-0.086-0.085-0.129-0.127 c-0.043-0.042-0.084-0.087-0.133-0.126c-0.062-0.051-0.135-0.092-0.201-0.133c-0.033-0.02-0.062-0.045-0.096-0.063l-14.777-7.73 c-0.531-0.277-1.16-0.282-1.695-0.016l-3.266,1.635l-9.062-4.479c-0.536-0.267-1.171-0.257-1.7,0.025L0.986,7.999 C0.373,8.325-0.008,8.967,0,9.662c0.008,0.695,0.402,1.327,1.023,1.641l15.518,7.83L2,26.986c-0.034,0.02-0.061,0.043-0.094,0.062 c-0.07,0.043-0.141,0.085-0.204,0.14c-0.05,0.041-0.091,0.088-0.136,0.133c-0.041,0.044-0.084,0.08-0.121,0.125 c-0.056,0.068-0.102,0.145-0.146,0.22c-0.018,0.026-0.039,0.052-0.055,0.081c-0.002,0.003-0.002,0.005-0.004,0.007 c-0.053,0.102-0.094,0.207-0.129,0.313c-0.02,0.064-0.031,0.133-0.045,0.197c-0.011,0.062-0.027,0.119-0.034,0.182 c-0.007,0.066-0.001,0.135-0.001,0.199c0.001,0.057-0.004,0.107,0.001,0.164c0.009,0.084,0.029,0.164,0.049,0.248 c0.009,0.033,0.011,0.069,0.021,0.104c0.036,0.113,0.084,0.229,0.142,0.34c0.001,0.002,0.002,0.004,0.002,0.006 c0.005,0.009,0.012,0.013,0.015,0.021c0.069,0.125,0.157,0.24,0.255,0.349c0.029,0.032,0.059,0.062,0.09,0.092 c0.083,0.079,0.175,0.149,0.273,0.215c0.031,0.021,0.061,0.043,0.094,0.062c0.012,0.006,0.021,0.018,0.033,0.021l18.472,9.893 c0.08,0.043,0.165,0.064,0.247,0.097c0.053,0.02,0.103,0.045,0.157,0.061c0.156,0.041,0.315,0.064,0.472,0.064 c0.002,0,0.004,0,0.004,0c0.15,0,0.307-0.021,0.459-0.062c0.051-0.012,0.098-0.035,0.146-0.055 c0.078-0.024,0.158-0.048,0.234-0.085l18.357-9.321c0.916-0.467,1.282-1.586,0.814-2.502c-0.021-0.043-0.052-0.078-0.076-0.119 c-0.113-0.526-0.449-1.006-0.972-1.27l-9.215-4.648l10.619-5.917c0.004-0.002,0.008-0.004,0.012-0.006l0.019-0.009 c0.021-0.015,0.043-0.033,0.065-0.048c0.077-0.049,0.152-0.098,0.229-0.158c0.041-0.034,0.073-0.077,0.112-0.116 c0.049-0.047,0.099-0.091,0.14-0.143c0.049-0.064,0.09-0.131,0.131-0.199c0.02-0.034,0.045-0.062,0.062-0.099 c0.002-0.003,0.003-0.007,0.004-0.01c0.044-0.081,0.072-0.166,0.104-0.253c0.074-0.219,0.104-0.443,0.098-0.666 c0-0.049,0.006-0.096,0-0.142C42.689,14.433,42.665,14.315,42.629,14.199z M5.905,9.595l6.222-3.318l5.892,2.912l-6.07,3.456 L5.905,9.595z M36.043,28.972l-14.662,7.444L6.817,28.617l13.776-7.442L36.043,28.972z M27.1,20.289l-5.674-2.861l-5.489-2.77 l6.926-3.942l3.191-1.599l10.881,5.691L27.1,20.289z"></path>{" "}
            </g>{" "}
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Premier Tiling Services",
      description:
        "At Kwix4 Building Services, we understand the intricate beauty of tiles and the transformation they bring to any space. Our expertise spans various tiling materials, ensuring that each project we undertake resonates with durability and aesthetic appeal.",
      features: [
        "Expertise in diverse tiling materials.",
        "Precision-cut tiles for impeccable finishes.",
        "Waterproofing solutions for bathrooms & kitchens.",
        "Comprehensive tiling solutions: From consultation to installation.",
      ],
      images:
        tilingImage,
    },
  },
  {
    name: "Flooring Services",
    url: "/services/flooring",
    description:
      "Enhancing homes and offices with sturdy, aesthetic flooring options that stand the test of time.",
    slug: "flooring",
    svg: (
      <>
        <svg
          fill="#ffffff"
          height="48px"
          width="48px"
          version="1.1"
          id="Layer_1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="-51.2 -51.2 614.40 614.40"
          stroke="#ffffff"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0">
            <rect
              x="-51.2"
              y="-51.2"
              width="614.40"
              height="614.40"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <g>
              {" "}
              <g>
                {" "}
                <path d="M495.304,0H16.696c-1.75,0-3.702,0.296-5.668,1.008C4.372,3.405-0.003,9.686,0,16.701c0,18.709,0,461.67,0,478.598 C-0.004,504.579,7.603,512,16.696,512h478.609c9.106,0,16.7-7.423,16.696-16.701c0-16.93,0-459.906,0-478.598 C512.004,7.613,504.614,0,495.304,0z M454.998,33.391L322.783,165.606l-43.172-43.172l89.044-89.044H454.998z M389.565,146.046 l43.172,43.172L322.783,299.172L279.611,256L389.565,146.046z M321.432,33.391L256,98.824l-65.432-65.432H321.432z M299.172,189.217L256,232.389L146.046,122.435l43.172-43.172L299.172,189.217z M232.389,256L122.435,365.954l-43.172-43.172 l109.954-109.954L232.389,256z M143.346,33.391l22.261,22.261l-43.172,43.172L57.002,33.391H143.346z M33.391,57.002 l132.215,132.215l-43.172,43.172l-89.044-89.044V57.002z M33.391,190.567L98.824,256l-65.432,65.433V190.567z M33.391,368.654 l22.261-22.261l43.172,43.172l-65.432,65.432V368.654z M57.002,478.609l132.215-132.215l43.172,43.172l-89.044,89.044H57.002z M190.567,478.609L256,413.176l65.433,65.432H190.567z M212.828,322.783L256,279.611l109.954,109.954l-43.172,43.172 L212.828,322.783z M368.654,478.609l-22.261-22.261l43.172-43.172l65.432,65.432H368.654z M478.609,454.998L346.394,322.783 l43.172-43.172l89.044,89.044V454.998z M478.609,321.432L413.176,256l65.432-65.432V321.432z M478.609,143.346l-22.261,22.261 l-43.172-43.172l65.432-65.432V143.346z"></path>{" "}
              </g>{" "}
            </g>{" "}
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Exceptional Flooring Solutions",
      description:
        "At Kwix4 Building Services, we believe the floor is not just the foundation of your space but also the foundation of your comfort and style. Our flooring services promise longevity, functionality, and elegance. From hardwood to laminate, from vinyl to tile, we offer a broad spectrum of options tailored to your needs and aesthetic preferences.",
      features: [
        "Mastery over diverse flooring types: hardwood, laminate, vinyl, and more.",
        "Installation processes that ensure longevity and durability.",
        "Solutions that blend functionality with style, ensuring your floor speaks volumes.",
        "Expert advice and guidance, from choosing the right material to post-installation care.",
      ],
      images:
       flooringImage,
    },
  },
  {
    name: "Plastering Services",
    url: "/services/plastering",
    description:
      "Providing smooth finishes with top-notch plastering services, ensuring walls and ceilings are primed for decoration.",
    slug: "plastering",
    svg: (
      <>
        <svg
          fill="#ffffff"
          height="48px"
          width="48px"
          version="1.1"
          id="Layer_1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="-51.2 -51.2 614.40 614.40"
          stroke="#ffffff"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0">
            <rect
              x="-51.2"
              y="-51.2"
              width="614.40"
              height="614.40"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <g>
              {" "}
              <g>
                {" "}
                <path d="M503.467,307.2h-51.2v-25.6c0-4.719-3.814-8.533-8.533-8.533H171.196c4.207-33.621,32.981-59.733,67.738-59.733h59.733 c0,4.719,3.814,8.533,8.533,8.533h145.067c4.719,0,8.533-3.814,8.533-8.533v-51.2c0-4.719-3.814-8.533-8.533-8.533H307.2 c-4.719,0-8.533,3.814-8.533,8.533h-59.733c-63.002,0-114.773,49.024-119.168,110.933H68.267c-4.719,0-8.533,3.814-8.533,8.533 v25.6h-51.2C3.814,307.2,0,311.014,0,315.733v34.133c0,4.719,3.814,8.533,8.533,8.533h494.933c4.719,0,8.533-3.814,8.533-8.533 v-34.133C512,311.014,508.186,307.2,503.467,307.2z M315.733,170.667h128V204.8h-128V170.667z M76.8,290.133H128 c4.719,0,8.533-3.814,8.533-8.533c0-56.465,45.935-102.4,102.4-102.4h59.733v17.067h-59.733 c-47.053,0-85.333,38.281-85.333,85.333c0,4.719,3.814,8.533,8.533,8.533H435.2V307.2H76.8V290.133z M494.933,341.333H17.067 v-17.067h477.867V341.333z"></path>{" "}
              </g>{" "}
            </g>{" "}
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Precision Plastering Services",
      description:
        "At Kwix4 Building Services, we recognize that smooth, flawless walls are the canvas for your interior design dreams. Our plastering services are backed by years of experience, ensuring that walls not only look impeccable but are also durable and long-lasting.",
      features: [
        "Advanced techniques for smooth and even plaster finishes.",
        "Use of high-quality materials ensuring durability and longevity.",
        "Specialized in both traditional and contemporary plastering styles.",
        "Seamless blending for repairs, ensuring your walls look as good as new.",
        "Expert advice on maintenance to keep your plaster in perfect condition.",
      ],
      images:
       plasteringImage,
    },
  },
  {
    name: "Painting Services",
    url: "/services/painting",
    description:
      "Transforming spaces with a splash of color; our painting services guarantee precision, cleanliness, and a perfect finish.",
    slug: "painting",
    svg: (
      <>
        <svg
          viewBox="-2.4 -2.4 28.80 28.80"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          stroke="#ffffff"
          width="48px"
          height="48px"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0">
            <rect
              x="-2.4"
              y="-2.4"
              width="28.80"
              height="28.80"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <path
              d="M8.46778 1.25H15.5322C15.972 1.24998 16.3514 1.24997 16.6627 1.27818C16.9918 1.30802 17.3178 1.37407 17.625 1.55144C17.967 1.74892 18.2511 2.03296 18.4486 2.375C18.6259 2.68221 18.692 3.00817 18.7218 3.33735C18.7333 3.46384 18.7401 3.60156 18.7441 3.75L19.069 3.75C19.4934 3.74999 19.8477 3.74999 20.1388 3.76897C20.4418 3.78872 20.7283 3.83128 21.0074 3.94117C21.7174 4.22069 22.2793 4.78261 22.5588 5.49258C22.6687 5.77171 22.7113 6.05823 22.731 6.3612C22.75 6.65235 22.75 7.00665 22.75 7.43105V7.4551C22.75 7.4958 22.75 7.53596 22.7501 7.57559C22.7506 8.23353 22.7511 8.74599 22.593 9.19346C22.3636 9.84304 21.8987 10.3829 21.2904 10.7061C20.8713 10.9288 20.3644 11.0044 19.7137 11.1014C19.6745 11.1073 19.6348 11.1132 19.5945 11.1192L15.5179 11.7307C14.6861 11.8555 14.1318 11.94 13.7209 12.0556C13.3303 12.1654 13.1623 12.2796 13.0528 12.4067C12.9236 12.5567 12.8363 12.7669 12.7902 13.2599C12.9356 13.2662 13.072 13.2765 13.1972 13.2933C13.5527 13.3411 13.9284 13.4535 14.2374 13.7626C14.5465 14.0716 14.6589 14.4473 14.7067 14.8028C14.7501 15.1256 14.7501 15.5224 14.75 15.9553V20.0447C14.7501 20.4776 14.7501 20.8744 14.7067 21.1972C14.6589 21.5527 14.5465 21.9284 14.2374 22.2374C13.9284 22.5465 13.5527 22.6589 13.1972 22.7067C12.8744 22.7501 12.4776 22.7501 12.0447 22.75H11.9553C11.5224 22.7501 11.1256 22.7501 10.8028 22.7067C10.4473 22.6589 10.0716 22.5465 9.76256 22.2374C9.45354 21.9284 9.34109 21.5527 9.2933 21.1972C9.2499 20.8744 9.24995 20.4776 9.25 20.0448V15.9553C9.24995 15.5224 9.2499 15.1256 9.2933 14.8028C9.34109 14.4473 9.45354 14.0716 9.76256 13.7626C10.0716 13.4535 10.4473 13.3411 10.8028 13.2933C10.949 13.2736 11.1105 13.2629 11.2836 13.257C11.3358 12.5533 11.4805 11.9337 11.9162 11.4278C12.2913 10.9923 12.7718 10.7643 13.3149 10.6116C13.8273 10.4675 14.4759 10.3702 15.2486 10.2543L19.372 9.63581C20.2001 9.5116 20.4254 9.46711 20.5865 9.38148C20.8631 9.23455 21.0744 8.98918 21.1786 8.69392C21.2394 8.52185 21.25 8.29243 21.25 7.4551C21.25 7.00044 21.2496 6.6953 21.2342 6.45878C21.2192 6.22858 21.1922 6.11609 21.1631 6.04208C21.0361 5.71937 20.7806 5.46395 20.4579 5.3369C20.3839 5.30776 20.2714 5.2808 20.0412 5.26579C19.8047 5.25037 19.4996 5.25 19.0449 5.25H18.7441C18.7401 5.39845 18.7333 5.53617 18.7218 5.66265C18.692 5.99183 18.6259 6.31779 18.4486 6.625C18.2511 6.96704 17.967 7.25108 17.625 7.44856C17.3178 7.62593 16.9918 7.69198 16.6627 7.72182C16.3514 7.75003 15.972 7.75002 15.5322 7.75H8.46777C8.02799 7.75002 7.64864 7.75003 7.33735 7.72182C7.00817 7.69198 6.68221 7.62593 6.375 7.44856C6.03296 7.25108 5.74892 6.96704 5.55144 6.625C5.37407 6.31779 5.30802 5.99183 5.27818 5.66265C5.26567 5.5246 5.25871 5.37317 5.25484 5.20902C4.96102 5.10744 4.75 4.82836 4.75 4.5C4.75 4.17164 4.96102 3.89257 5.25484 3.79099C5.25871 3.62683 5.26567 3.4754 5.27818 3.33735C5.30802 3.00817 5.37407 2.68221 5.55144 2.375C5.74892 2.03296 6.03296 1.74892 6.375 1.55144C6.68221 1.37407 7.00817 1.30802 7.33735 1.27818C7.64864 1.24997 8.02799 1.24998 8.46778 1.25ZM12.0047 14.75C12.0031 14.75 12.0016 14.75 12 14.75C11.5074 14.75 11.2134 14.7516 11.0027 14.7799C10.9061 14.7929 10.8577 14.8082 10.836 14.8172C10.8309 14.8192 10.8276 14.8209 10.8257 14.8219L10.8232 14.8232L10.8219 14.8257C10.8209 14.8276 10.8192 14.8309 10.8172 14.836C10.8082 14.8577 10.7929 14.9061 10.7799 15.0027C10.7516 15.2134 10.75 15.5074 10.75 16V20C10.75 20.4926 10.7516 20.7866 10.7799 20.9973C10.7929 21.0939 10.8082 21.1423 10.8172 21.164C10.8192 21.1691 10.8209 21.1724 10.8219 21.1743L10.8232 21.1768L10.8257 21.1781C10.8276 21.1792 10.8309 21.1808 10.836 21.1828C10.8577 21.1918 10.9061 21.2071 11.0027 21.2201C11.2134 21.2484 11.5074 21.25 12 21.25C12.4926 21.25 12.7866 21.2484 12.9973 21.2201C13.0939 21.2071 13.1423 21.1918 13.164 21.1828C13.1676 21.1814 13.1703 21.1801 13.1723 21.1792C13.1731 21.1788 13.1738 21.1784 13.1743 21.1781L13.1768 21.1768L13.1781 21.1743C13.1791 21.1724 13.1808 21.1691 13.1828 21.164C13.1918 21.1423 13.2071 21.0939 13.2201 20.9973C13.2484 20.7866 13.25 20.4926 13.25 20V16C13.25 15.5074 13.2484 15.2134 13.2201 15.0027C13.2071 14.9061 13.1918 14.8577 13.1828 14.836C13.1808 14.8309 13.1791 14.8276 13.1781 14.8257L13.1768 14.8232L13.1743 14.8219C13.1731 14.8212 13.1712 14.8203 13.1687 14.8191C13.1673 14.8185 13.1658 14.8179 13.164 14.8172C13.1423 14.8082 13.0939 14.7929 12.9973 14.7799C12.7873 14.7517 12.4945 14.75 12.0047 14.75ZM7.47275 2.77206C7.2476 2.79247 7.16586 2.82689 7.125 2.85048C7.01099 2.91631 6.91631 3.01099 6.85048 3.125C6.82689 3.16586 6.79246 3.2476 6.77206 3.47275C6.75072 3.7082 6.75 4.01889 6.75 4.5C6.75 4.98111 6.75072 5.2918 6.77206 5.52726C6.79246 5.7524 6.82689 5.83414 6.85048 5.875C6.91631 5.98902 7.01099 6.08369 7.125 6.14952C7.16586 6.17311 7.2476 6.20754 7.47275 6.22794C7.7082 6.24928 8.01889 6.25 8.5 6.25H15.5C15.9811 6.25 16.2918 6.24928 16.5273 6.22794C16.7524 6.20754 16.8341 6.17311 16.875 6.14952C16.989 6.08369 17.0837 5.98902 17.1495 5.875C17.1731 5.83414 17.2075 5.7524 17.2279 5.52726C17.2493 5.2918 17.25 4.98111 17.25 4.5C17.25 4.01889 17.2493 3.7082 17.2279 3.47275C17.2075 3.2476 17.1731 3.16587 17.1495 3.125C17.0837 3.01099 16.989 2.91631 16.875 2.85048C16.8341 2.82689 16.7524 2.79247 16.5273 2.77206C16.2918 2.75072 15.9811 2.75 15.5 2.75H8.5C8.01889 2.75 7.7082 2.75072 7.47275 2.77206ZM10.8232 14.8232C10.8236 14.823 10.8236 14.8228 10.8232 14.8232C10.8228 14.8237 10.823 14.8236 10.8232 14.8232Z"
              fill="#ffffff"
            ></path>{" "}
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Flawless Painting Services",
      description:
        "At Kwix4 Building Services, every stroke of our brush is a commitment to perfection. We transform spaces with colors that not only resonate with your personality but also stand the test of time. Our comprehensive painting solutions ensure every corner shines with precision.",
      features: [
        "Expertise in a range of painting techniques, both modern and traditional.",
        "Use of premium-quality, eco-friendly paints ensuring vibrant and lasting colors.",
        "Detailed preparation and finishing, ensuring a flawless look.",
        "Capability to handle diverse surfaces – from walls to ceilings, exteriors to interiors.",
        "Personalized consultations to pick the perfect palette for your space.",
      ],
      images:
        paintingImage,
    },
  },
  {
    name: "Bathroom Renovations",
    url: "/services/bathroom-renovations",
    description:
      "Revitalizing old bathrooms with modern designs and fixtures, making them both functional and luxurious.",
    slug: "bathroom-renovations",
    svg: (
      <>
        <svg
          fill="#000000"
          viewBox="-2.4 -2.4 28.80 28.80"
          id="bathroom"
          data-name="Flat Line"
          xmlns="http://www.w3.org/2000/svg"
          className="icon flat-line"
          width="48px"
          height="48px"
        >
          <g id="SVGRepo_bgCarrier">
            <rect
              x="-2.4"
              y="-2.4"
              width="28.80"
              height="28.80"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g id="SVGRepo_tracerCarrier"></g>
          <g id="SVGRepo_iconCarrier">
            <path
              id="secondary"
              d="M11,10H5V9A3,3,0,0,1,8,6H8a3,3,0,0,1,3,3Z"
              style={{ fill: " #ffffff", strokeWidth: 2 }}
            ></path>
            <path
              id="primary"
              d="M17,21V5a2,2,0,0,0-2-2H10A2,2,0,0,0,8,5V6"
              style={{
                fill: "none",
                stroke: "#ffffff",
                strokeLinecap: "round",
                strokeLinejoin: "round",
                strokeWidth: "2",
              }}
            ></path>
            <path
              id="primary-2"
              data-name="primary"
              d="M11,10H5V9A3,3,0,0,1,8,6H8a3,3,0,0,1,3,3Zm4,11h4M6,14v2m4-2v2"
              style={{
                fill: "none",
                stroke: "#ffffff",
                strokeLinecap: "round",
                strokeLinejoin: "round",
                strokeWidth: "2px",
              }}
            ></path>
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Bathroom Renovation Mastery",
      description:
        "At Kwix4, we elevate bathrooms from functional to fantastic. Start and end your day in a space tailored for comfort and style.",
      features: [
        "Maximized space utilization.",
        "Modern and traditional designs.",
        "Durable materials and finishes.",
        "Precision in tiling and fixture installation.",
        "Transparent communication, timely delivery.",
      ],
      images:
        bathroomImage,
    },
  },
  {
    name: "Kitchen Installations",
    url: "/services/kitchen-installations",
    description:
      "Designing and installing modern, ergonomic kitchens that are a blend of aesthetics and functionality.",
    slug: "kitchen-installations",
    svg: (
      <>
        <svg
          fill="#ffffff"
          viewBox="-4.81 -4.81 57.71 57.71"
          xmlns="http://www.w3.org/2000/svg"
          data-name="Слой 1"
          stroke="#ffffff"
          width="48px"
          height="48px"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0">
            <rect
              x="-4.81"
              y="-4.81"
              width="57.71"
              height="57.71"
              rx="0"
              fill="#FB923C"
            ></rect>
          </g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            <path d="M9 0 A 1.0001 1.0001 0 0 0 8 1L8 4.484375L1.4179688 9.1855469 A 1.0001 1.0001 0 0 0 1 10L1 14 A 1.0001 1.0001 0 0 0 2 15L24 15 A 1.0001 1.0001 0 0 0 25 14L25 10 A 1.0001 1.0001 0 0 0 24.582031 9.1855469L18 4.4863281L18 1 A 1.0001 1.0001 0 0 0 17 0L9 0 z M 10 2L16 2L16 5 A 1.0001 1.0001 0 0 0 16.417969 5.8144531L23 10.513672L23 13L3 13L3 10.513672L9.5820312 5.8144531 A 1.0001 1.0001 0 0 0 10 5L10 2 z M 5.5 21C4.8457598 21 4.2978026 21.418077 4.0917969 22L1 22 A 1.0001 1.0001 0 0 0 0 23L0 45 A 1.0001 1.0001 0 0 0 1 46L47.095703 46 A 1.0001 1.0001 0 0 0 48.095703 45L48.095703 23 A 1.0001 1.0001 0 0 0 47.095703 22L21.908203 22C21.702197 21.418077 21.15424 21 20.5 21L5.5 21 z M 2 24L5.5 24L20.5 24L24.095703 24L24.095703 44L2 44L2 24 z M 26.095703 24L46.095703 24L46.095703 30L26.095703 30L26.095703 24 z M 5 26 A 1.0001 1.0001 0 0 0 4 27L4 41 A 1.0001 1.0001 0 0 0 5 42L21 42 A 1.0001 1.0001 0 0 0 22 41L22 27 A 1.0001 1.0001 0 0 0 21 26L5 26 z M 30 26 A 1.0001 1.0001 0 1 0 30 28L42 28 A 1.0001 1.0001 0 1 0 42 26L30 26 z M 6 28L20 28L20 40L6 40L6 28 z M 26.095703 32L46.095703 32L46.095703 44L26.095703 44L26.095703 32 z M 29.984375 34.986328 A 1.0001 1.0001 0 0 0 29 36L29 41 A 1.0001 1.0001 0 1 0 31 41L31 36 A 1.0001 1.0001 0 0 0 29.984375 34.986328 z"></path>
          </g>
        </svg>
      </>
    ),
    details: {
      title: "Kitchen Installation Excellence",
      description:
        "With Kwix4, step into kitchens designed for functionality and flair. Experience culinary delights in spaces tailored just for you.",
      features: [
        "Customized solutions for all kitchen sizes.",
        "Integration of top-tier appliances.",
        "High-quality cabinetry and countertop installations.",
        "Efficient space planning for maximum utility.",
        "Quick, hassle-free installations.",
      ],
      images: kitchenImage,
    },
  },
];
